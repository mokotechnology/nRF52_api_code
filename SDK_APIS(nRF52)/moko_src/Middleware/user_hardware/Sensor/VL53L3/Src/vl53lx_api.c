...
VL53LX_Error VL53L1X_GetDistanceMode(VL53LX_DEV dev, uint16_t *DM)
{
	uint8_t TempDM, status=0;

	status |= VL53LX_RdByte(dev,PHASECAL_CONFIG__TIMEOUT_MACROP, &TempDM);
	if (TempDM == 0x14)
		*DM=1;
	if(TempDM == 0x0A)
		*DM=2;
...
VL53LX_Error VL53L1X_GetTimingBudgetInMs(VL53LX_DEV dev, uint16_t *pTimingBudget)
{
	uint16_t Temp;
	VL53LX_Error status = 0;

	status |= VL53LX_RdWord(dev, RANGE_CONFIG__TIMEOUT_MACROP_A_HI, &Temp);
	switch (Temp) {
		case 0x001D :
			*pTimingBudget = 15;
			break;
...
VL53LX_Error VL53L1X_StartRanging(VL53LX_DEV dev)
{
	VL53LX_Error status = 0;

	status |= VL53LX_WrByte(dev, SYSTEM__MODE_START, 0x40);	/* Enable VL53L1X */
	return status;
}

VL53LX_Error VL53L1X_StopRanging(VL53LX_DEV dev)
{
	VL53LX_Error status = 0;

	status |= VL53LX_WrByte(dev, SYSTEM__MODE_START, 0x00);	/* Disable VL53L1X */
	return status;
}


VL53LX_Error VL53L1X_SetDistanceMode(VL53LX_DEV dev, uint16_t DM)
{
	uint16_t TB;
	VL53LX_Error status = 0;
	...
	switch (DM) {
	case 1:
		status = VL53LX_WrByte(dev, PHASECAL_CONFIG__TIMEOUT_MACROP, 0x14);
		status = VL53LX_WrByte(dev, RANGE_CONFIG__VCSEL_PERIOD_A, 0x07);
		status = VL53LX_WrByte(dev, RANGE_CONFIG__VCSEL_PERIOD_B, 0x05);
		status = VL53LX_WrByte(dev, RANGE_CONFIG__VALID_PHASE_HIGH, 0x38);
		status = VL53LX_WrWord(dev, SD_CONFIG__WOI_SD0, 0x0705);
		status = VL53LX_WrWord(dev, SD_CONFIG__INITIAL_PHASE_SD0, 0x0606);
		break;
	case 2:
		status = VL53LX_WrByte(dev, PHASECAL_CONFIG__TIMEOUT_MACROP, 0x0A);
		status = VL53LX_WrByte(dev, RANGE_CONFIG__VCSEL_PERIOD_A, 0x0f);
		status = VL53LX_WrByte(dev, RANGE_CONFIG__VCSEL_PERIOD_B, 0x0d);
		status = VL53LX_WrByte(dev, RANGE_CONFIG__VALID_PHASE_HIGH, 0xb8);
		status = VL53LX_WrWord(dev, SD_CONFIG__WOI_SD0, 0x0f0D);
		status = VL53LX_WrWord(dev, SD_CONFIG__INITIAL_PHASE_SD0, 0x0e0e);
		break;
	default:
		status = 1;
		break;
...
VL53LX_Error VL53L1X_SetTimingBudgetInMs(VL53LX_DEV dev, uint16_t TimingBudgetInMs)
{
	uint16_t DM;
	VL53LX_Error  status=0;
	...
	if (DM == 0)
		...
		case 15: /* only available in short distance mode */
			VL53LX_WrWord(dev, RANGE_CONFIG__TIMEOUT_MACROP_A_HI,
					0x01D);
			VL53LX_WrWord(dev, RANGE_CONFIG__TIMEOUT_MACROP_B_HI,
					0x0027);
			break;
		case 20:
			VL53LX_WrWord(dev, RANGE_CONFIG__TIMEOUT_MACROP_A_HI,
...
VL53LX_Error VL53L1X_SetInterMeasurementInMs(VL53LX_DEV dev, uint32_t InterMeasMs)
{
	uint16_t ClockPLL;
	VL53LX_Error status = 0;

	status |= VL53LX_RdWord(dev, VL53L1_RESULT__OSC_CALIBRATE_VAL, &ClockPLL);
	ClockPLL = ClockPLL&0x3FF;
	VL53LX_WrDWord(dev, VL53L1_SYSTEM__INTERMEASUREMENT_PERIOD,
			(uint32_t)(ClockPLL * InterMeasMs * 1.075));
	return status;

}

VL53LX_Error VL53L1X_ClearInterrupt(VL53LX_DEV dev)
{
	VL53LX_Error status = 0;

	status |= VL53LX_WrByte(dev, SYSTEM__INTERRUPT_CLEAR, 0x01);
	return status;
}

VL53LX_Error VL53L1X_GetInterruptPolarity(VL53LX_DEV dev, uint8_t *pInterruptPolarity)
{
	uint8_t Temp;
	VL53LX_Error status = 0;

	status |= VL53LX_RdByte(dev, GPIO_HV_MUX__CTRL, &Temp);
	Temp = Temp & 0x10;
	*pInterruptPolarity = !(Temp>>4);
	return status;
}

VL53LX_Error VL53L1X_CheckForDataReady(VL53LX_DEV dev, uint8_t *isDataReady)
{
	uint8_t Temp;
	uint8_t IntPol;
	VL53LX_Error status = 0;
	...
	status |= VL53LX_RdByte(dev, GPIO__TIO_HV_STATUS, &Temp);
...
VL53LX_Error VL53L1X_SensorInit(VL53LX_DEV dev)
{
	VL53LX_Error status = 0;
	uint8_t Addr = 0x00, tmp;

	for (Addr = 0x2D; Addr <= 0x87; Addr++){
		status |= VL53LX_WrByte(dev, Addr, VL51L1X_DEFAULT_CONFIGURATION[Addr - 0x2D]);
	}
	...
	status |= VL53LX_WrByte(dev, VL53L1_VHV_CONFIG__TIMEOUT_MACROP_LOOP_BOUND, 0x09); /* two bounds VHV */
	status |= VL53LX_WrByte(dev, 0x0B, 0); /* start VHV from the previous temperature */
	return status;
}

VL53LX_Error VL53L1X_GetRangeStatus(VL53LX_DEV dev, uint8_t *rangeStatus)
{
	VL53LX_Error status = 0;
	uint8_t RgSt;

	*rangeStatus = 255;
	status |= VL53LX_RdByte(dev, VL53L1_RESULT__RANGE_STATUS, &RgSt);
	RgSt = RgSt & 0x1F;
	if (RgSt < 24)
...
VL53LX_Error VL53L1X_GetDistance(VL53LX_DEV dev, uint16_t *distance)
{
	VL53LX_Error status = 0;
	uint16_t tmp;
...
VL53LX_Error VL53L1X_BootState(VL53LX_DEV dev, uint8_t *state)
{
	VL53LX_Error status = 0;
	uint8_t tmp = 0;

	status |= VL53LX_RdByte(dev,VL53L1_FIRMWARE__SYSTEM_STATUS, &tmp);
	*state = tmp;
	return status;
}
